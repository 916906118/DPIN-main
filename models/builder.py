...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        if exp_mode == 'cnn':
            logger.info('Ablation: Using CNN Backbone (Dual ResNet50)')
            from .ablations import DualResNet
            self.channels = [256, 512, 1024, 2048] # 对应 ResNet 输出通道
            self.backbone = DualResNet()
        elif exp_mode == 'transformer':
            logger.info('Ablation: Using Transformer Backbone (Dual Swin-Tiny)')
            from .ablations import DualSwin
            self.channels = [96, 192, 384, 768] # 对应 Swin-T 输出通道
            self.backbone = DualSwin()
        else:
            # --- [原代码保持不变] ---
